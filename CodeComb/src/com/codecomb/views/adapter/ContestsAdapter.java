...
public class ContestsAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = layoutInflater.inflate(R.layout.lv_item_contests,
					null);

			viewHolder = new ViewHolder();

			viewHolder.vContestStatus = (TextView) convertView
					.findViewById(R.id.vContestStatus);
			viewHolder.vContestTitle = (TextView) convertView
					.findViewById(R.id.vContestTitle);
		...
		Date begin = contests.get(position).getBegin();

		Date end = contests.get(position).getEnd();

		// int compare = .compareTo(new Date().getTime());
		...
		if (begin.getTime() > System.currentTimeMillis()) {
			...
			int color = context.getResources().getColor(
					R.color.col_contest_status_pending);
			viewHolder.vContestStatus.setTextColor(color);

		} else if (end.getTime() > System.currentTimeMillis()) {

			viewHolder.vContestStatus.setText("Live");
			int color = context.getResources().getColor(
					R.color.col_contest_status_live);
			viewHolder.vContestStatus.setTextColor(color);
			...
			int color = context.getResources().getColor(
					R.color.col_contest_status_ended);
			viewHolder.vContestStatus.setTextColor(color);
